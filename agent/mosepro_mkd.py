...
from modules.WKD import WKDLoss
class MOSE(object):
    def __init__(self, model:nn.Module, buffer, optimizer, input_size, args):
        ...
        if self.alpha_min is None or self.alpha_max is None:
            ...
            self.ema_alphas[0] = self.ema_alpha1
            self.ema_alphas[1] = self.ema_alpha2
            self.ema_alphas[2] = self.ema_alpha3
            self.ema_alphas[3] = self.ema_alpha4
        else:
            # Automatically set a specific number of teachers
            for i in range(self.n_teacher):
                self.ema_models[i] = deepcopy(self.model)
                self.ema_alphas[i] = 10**(
                    np.log10(self.alpha_min) + 
                    (np.log10(self.alpha_max) - np.log10(self.alpha_min))*i/(max(1,self.n_teacher - 1))
                    )
    ...
    def train_any_task(self, task_id, train_loader, epoch):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if len(self.buffer) > 0:
                ...
                with autocast():
                    ...
                    if task_id > 0:
                        ...
                        combined_x = torch.cat([buffer_x, x]).cuda()
                        combined_y = torch.cat([buffer_y, y]).cuda()
                        combined_aug = self.transform(combined_x)
                        logits_stu = self.model.features(combined_aug)
                        logits_stu_raw = self.model.features(combined_x)
                        for teacher in self.ema_models.values():
                            logits_tea = teacher.features(combined_aug)
                        for i in range(len(feat_list)):
                            feat = feat_list[i]
                            proj = proj_list[i]
                            proj_x = proj[:new_x.shape[0]]
                            proj_x_r = proj[new_x.shape[0]:]
                            pred = pred_list[i]
                            ...
                            # buffer_y = mem_y
                            # buffer_x_pair = torch.cat([buffer_x, self.transform(buffer_x)], dim=0)
                            proto_seen_loss, cur_buffer_z1_proto, cur_buffer_z2_proto, cur_buffer_z = self.cal_buffer_proto_loss(buffer_x, buffer_y, buffer_x_pair, task_id, feat_id=i)
                            ...
                            loss_dist += (self.wkdloss(logits_tea[i].detach(), logits_stu[i]) + self.wkdloss(logits_tea[i].detach(), logits_stu_raw[i]))/2
                            loss_dist = loss_dist / len(self.ema_models)
                            
                            loss_mkd = self.kd_lambda*loss_dist

                            loss += ins_loss + 2 * ce_loss + distill_loss + OPE_loss + loss_mkd
                            ...
                            loss_log['train/ope'] += OPE_loss.item() if OPE_loss != 0. else 0.
                            loss_log['train/mkd'] += loss_mkd.item() if loss_mkd != 0. else 0.
                    else:
                        # rotate and augment
                        new_x = RandomFlip(new_x, 2)
                        new_y = new_y.repeat(2)
            ...
            if num_d % self.print_num == 0 or batch_idx == 1:
                print(f"==>>> it: {batch_idx}, loss: ins {loss_log['train/ins']:.2f} + ce {loss_log['train/ce']:.3f} + distill {loss_log['train/distill']:.3f} + ope {loss_log['train/ope']:.3f} + mkd {loss_log['train/mkd']:.3f} = {loss:.6f}, {100 * (num_d / self.total_samples)}%")
    ...
    def test_model(self, loader, i, feat_id):
        ...
        if self.use_ncm:
            ...
            for batch_idx, (data, target) in enumerate(loader):
                ...
                pred = dists.min(1)[1]
                class_holder = torch.Tensor(self.class_holder).to(data.device)
                pred = class_holder[pred]
    ...
    def cal_buffer_proto_loss(self, buffer_x, buffer_y, buffer_x_pair, task_id, feat_id):
        buffer_fea, buffer_z = self.model(buffer_x_pair, use_proj=True)
        buffer_z_norm = F.normalize(buffer_z[feat_id])
    ...
    def update_ema(self, init=False):
        ...
        for i, ema_model in enumerate(self.ema_models.values()):
            ...
            for param, ema_param in zip(self.model.parameters(), ema_model.parameters()):
                ...
                if init:
                    ema_param.data.mul_(0).add_(p * alpha / (1 - alpha ** max(1, self.stream_idx // (self.args.batch_size * self.ema_correction_step))))
                else:
                    ema_param.data.mul_(1 - alpha).add_(p * alpha / (1 - alpha ** max(1, self.stream_idx // (self.args.batch_size * self.ema_correction_step))))
