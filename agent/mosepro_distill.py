    ...
    def train_any_task(self, task_id, train_loader, epoch):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
                    ...
                    if task_id > 0:
                        ...
                        for i in range(len(feat_list)):
                            ...
                            if i != self.expert:
                                ...
                                # ========== 新增：层级特征的“双蒸馏”示例 ========== 
                                # 假设 feat, stu_feat 格式 [B, C, H, W]，我们对 "通道维" & "空间维" 分别做对齐

                                # 1) 通道维度蒸馏 (Channel-wise)
                                #    沿 H,W 做平均 -> [B, C]，再对每个通道对齐
                                feat_s_channel = stu_feat.mean(dim=(2, 3))   # [B, C]
                                feat_t_channel = feat.detach().mean(dim=(2, 3))

                                dist_channel = torch.norm(
                                    F.normalize(feat_s_channel, dim=1) - F.normalize(feat_t_channel, dim=1),
                                    p=2, dim=1
                                ).mean()

                                # 2) 空间维度蒸馏 (Spatial-wise)
                                #    沿通道 C 做平均 -> [B,1,H,W]，再比较空间分布
                                feat_s_spatial = stu_feat.mean(dim=1, keepdim=True)  # [B,1,H,W]
                                feat_t_spatial = feat.detach().mean(dim=1, keepdim=True)

                                # 这里可以视需求选择是否做 normalize
                                dist_spatial = (F.normalize(feat_s_spatial, dim=(2,3)) -
                                                F.normalize(feat_t_spatial, dim=(2,3))).pow(2).mean()

                                # ========== 将上述几项合并到一个 distill_loss ========== 
                                alpha, beta, gamma = 1.0, 1.0, 1.0  # 超参权重示例
                                distill_loss = beta * dist_channel + gamma * dist_spatial
                            # prototype loss
                            buffer_x = mem_x
                            buffer_y = mem_y
                            ...
                            loss += ins_loss + ce_loss * 2 + distill_loss + OPE_loss 
            ...
            if num_d % self.print_num == 0 or batch_idx == 1:
                print(f"==>>> it: {batch_idx}, loss: ins {loss_log['train/ins']:.2f} + ce {loss_log['train/ce']:.3f} + distill {loss_log['train/distill']:.3f} + ope {loss_log['train/ope']:.3f} = {loss:.6f}, {100 * (num_d / self.total_samples)}%")
