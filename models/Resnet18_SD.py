...
class ResNetSD(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, *args, **kwargs) -> None:
        """
        ...
        self.attention_layers = nn.ModuleList([
            ...
            self._make_attention_layer(nf * 4 * block.expansion, size=True),
        ])
    ...
    def _make_attention_layer(self, planes, size=False):
        if size and self.args[0].dataset == 'mini_imagenet':
            layers = [
...
def resnet18_sd(nclasses: int, nf: int = 64, args=None, **kwargs) -> ResNetSD:
    """
    Instantiates a ResNet18 network.
    :param nclasses: number of output classes
    :param nf: number of filters
    :return: ResNet network
    """
    return ResNetSD(BasicBlock, [2, 2, 2, 2], nclasses, nf, args, **kwargs)